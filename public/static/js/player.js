...
(function($){
    ...
    ctx.updateCoverState = function(derection, preLoad){
        var speed = 800,
            defualtUrl = 'http://fm.illn.in/play/static/song.png',
            updateAlbumImgs = function updateAlbumImgs() {
                // ctx.diskCovers.attr('src', "http://fm.illn.in/img/?fm=" + ctx.currentSong.al.picUrl);
                ctx.diskCovers.attr('src', myPlay + '?pic=' + ctx.currentSong.pic_id)
            };
    ...
    ctx.updateSong = function(){
        ...
        $.ajax({
            url: myPlay + '?song=' + ctx.currentSong.id,
            type: 'GET',
            async: false,
            dataType: 'json',
        ...
        // ctx.player.fm = ctx.currentSong.mp3Url;
    ...
    ctx.setLyric = function(lrc_text){
        var lrc_map = {};
        var rows = (lrc_text.lyric).split('\n');
        var time_reg = /\[\d*:\d*((\.|:)\d*)*]/g;

        rows.forEach(function(row){
            var time_arr = row.match(time_reg);
            if (!time_arr) return;
            var text = row.replace(time_reg, '').trim();
            time_arr.forEach(function(time){
                var min = +time.match(/\[\d*/g)[0].slice(1),
                    sec = +time.match(/:\d*/g)[0].slice(1);
    ...
    ctx.curLyric = function(sec){
        var time_arr = Object.keys(ctx.lrcMap);
        time_arr.sort(function(pre, next){
            return +pre < +next ? 1 : -1
        });
        var ret = {};
    ...
    ctx.updatePic = function(){
        // $(".bg").css('background-image', 'url(http://fm.illn.in/img/?fm=' + ctx.currentSong.al.picUrl + ')');
        $('.bg').css('background-image', 'url(' + myPlay + '?pic=' + ctx.currentSong.pic_id + ')')
    };
    ...
    ctx.initProcessBtn = function($btn){
        var moveFun = function moveFun(e){
                var duration = ctx.player.duration,
                    e = e.originalEvent,
                    totalWidth = ctx.$processBar.width(),
                    percent,
                    moveX,
                    newWidth;
                e.preventDefault();
    ...
    ctx.initPlay = function (param) {
        var url = myPlay + '?id=' + param,
            nowDay = new Date().getDate();

        if (myList == '') {
            url = myPlay + '?song=' + mySong
        }
